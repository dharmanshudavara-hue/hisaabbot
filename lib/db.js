  ...
  if (supabase) {
    ...
    if (error && error.code === '42883') {
      const fallback = await supabase
        ...
        .select('user_id, username, pin_hash, recovery_key_hash, role, created_at')
        .single();
...
export async function syncWithSupabase() {
  ...
  // Pull transactions for the current user from cloud
  let pulled = 0;
  ...
  if (currentUser) {
    const { data: cloudTx, error: pullError } = await supabase
      .from('transactions')
      .select('*')
      .eq('user_id', currentUser.userId);
      
    if (!pullError && cloudTx && cloudTx.length > 0) {
      const db = await openDB();
