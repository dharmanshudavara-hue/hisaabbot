 ...
 * @param {number} monthlyRate - Monthly interest rate as a percentage (e.g. 2 for 2%)
 * @param {string} createdAt - ISO timestamp when the loan was created
 * @returns {{ interest: number, totalWithInterest: number, monthsElapsed: number }}
 */
export function calculateInterest(principal, monthlyRate, createdAt) {
  if (!principal || !monthlyRate || !createdAt) {
    return { interest: 0, totalWithInterest: principal || 0, monthsElapsed: 0 };
  }
  ...
  const monthsElapsed = diffMs / (1000 * 60 * 60 * 24 * 30.44); // Average days per month
  
  if (monthsElapsed <= 0) {
    return { interest: 0, totalWithInterest: principal, monthsElapsed: 0 };
  }

  // Simple interest: I = P × R × T / 100
  const interest = Math.round(principal * (monthlyRate / 100) * monthsElapsed);
