...
/**
 * Generate a unique User ID with timestamp + random suffix to prevent collisions
 * Format: HB-001-<timestamp>-<random>
 * This ensures uniqueness even under concurrent registrations
...
export function generateUserId(existingUsers = []) {
  const maxNum = existingUsers.reduce((max, u) => {
    // Match both old format (HB-001) and new format (HB-001-<timestamp>-<random>)
  ...
  const sequentialId = `HB-${String(nextNum).padStart(3, '0')}`;
  
  // Add timestamp and random suffix for uniqueness during concurrent operations
  const timestamp = Date.now().toString(36);
  const randomSuffix = Math.random().toString(36).substring(2, 8);
  
  return `${sequentialId}-${timestamp}-${randomSuffix}`;
}
